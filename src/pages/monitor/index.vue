    ...
    <el-row :gutter="20">
      <el-col :span="8" v-for="(status, index) in machineStatuses" :key="index">
        <el-card>
          ...
          <!-- 添加发送指令按钮 -->
          <el-button type="info" @click="sendCommand({ machine_id: status.machine_id, command: 'your_command'})" :disabled="!status.active">
            发送指令
          </el-button>
    ...
    for await (const frame of getMachineScreens(params)) {
      ...
      if (index !== -1) {
        ...
        if (frame.frameData) {
          ...
          // 若 frameData 为空，保留上一帧数据
          if (currentFrames.value[index]) {
            console.log('frameData 为空，使用上一帧数据');
          }
        }
      }
    }
